...
func putFilesToPasteboard(files fileArray:[String]) throws {
    ...
    let pasteboardItems = fileArray.map { path -> NSPasteboardItem in
        ...
        item.setString(path, forType: NSPasteboard.PasteboardType("public.utf8-plain-text"))
        return item
    }
    // write to pasteboard
...
func main() {
    do {
        ...
        // putFilesToPasteboard(allLines)
        ///
        var pathArray = Array(CommandLine.arguments.dropFirst())
        pathArray = pathArray.map {path -> String in
            return URL(fileURLWithPath: path).standardizedFileURL.path
        }
        try putFilesToPasteboard(files: pathArray)
    } catch {
