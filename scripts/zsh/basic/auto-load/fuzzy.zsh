...
fz() {
    ...
    dvar opts
    fzf-noempty --bind 'tab:toggle,shift-tab:toggle+beginning-of-line+kill-line,?:toggle-preview,ctrl-j:toggle+beginning-of-line+kill-line,ctrl-t:top,ctrl-a:select-all' --color=light --exit-0 --multi --hscroll-off 99999 "${(@)opts}" "$@"
    # --select-1 
}
fzf-noempty() {
    local in="$(</dev/stdin)"
