...
#: which agent session it shows. [agfi:h-agent-session-pick-overlay]
...
#: from [agfi:h-agent-session-pick-rows], and the choice goes back to
...
#: [agfi:h-agent-session-preview-cmd].
#:
...
#: same way, once --- see [agfi:h-agent-session-open-cmd]. Enter still
#: opens under the *tab's* key, this under the *transcript's*, which is why the
#: two do not cancel each other.
#:
#: Row layout, tab separated: window id, transcript, agent, label, profile, last
...
selected="$(print -r -- "${rows}" |
    fzf --delimiter=$'\t' --with-nth='4..' --no-multi --ansi \
