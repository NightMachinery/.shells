...
#: [help:night/agsi]
aliasfn-ng agsi ugbase_follow=n nightNotes="$NIGHTDIR" ntsearch_additional_paths=(~/.zshenv ~/.zshrc ~/.shared.sh ~/.localScripts ~/.glances ~/.vimrc ~/.ideavimrc ~/.tmux.conf ~/.privateBTT.sh ~/.privateShell ~/.privateStartup.sh ~/.startup..private..zsh ~/.hammerspoon/init.lua ~/test_nonexistent) ntsearch_glob='' ntl-rg # ntsearch_glob=$textglob
##
function agfi1() {
    ##
...
function ntsearch_() {
    ...
    ntsearch_fd | {
        ...
        @opts query "$query" dir_main "$nightNotes" previewcode "${previewcode}" preview_header_lines "${preview_header_lines}" hidden "${hidden}" delim_opts [ "${delim_opts[@]}" ] opts [ "${fzopts[@]}" ] pattern "${ntsearch_lines_pattern}" @ h-grep-output-to-fz
    }
}
