...
rsp-safe --rsync-path="mkdir -p ~/bin && rsync" "$NIGHTDIR/zshlang/wrappers/brishz/brishzq.zsh" "$NIGHTDIR/python/telegram-send/tsend.py" "$NIGHTDIR/perllang/ssh_password_auth_disable.pl" ${fullhost}:bin/

rsp-safe2 ${fullhost}:bin/ $NIGHTDIR/perllang/url_dir_count.pl $NIGHTDIR/python/{Sharif/SID,codex_status,codex_loop,torch_cat}.py
#+end_src
...
** rsync from a setup server to another with net blackout
#+begin_src zsh :eval never
fullhost=m201

rsp-safe ~/.local/share/zinit "${fullhost}:.local/share/"

rsp-safe ~/.local/bin/ "${fullhost}:.local/bin/"

rsp-safe "$(realpath "${commands[gost]}")" "${fullhost}:bin/"
rsp-safe ~/bin/ "${fullhost}:bin/"

#+end_src

#+begin_src zsh :eval never
rsp-safe ~/.base/{cf-proxy,startup.zsh} "${fullhost}:.base/"
#+end_src

From laptop:
#+begin_src zsh :eval never
rsp-safe ~/base/Software/Linux/gost "${fullhost}:bin/"

rsp-safe $NIGHTDIR/python/Sharif/sharif_net_login.zsh "${fullhost}:.base/"
#+end_src
...
function gost-setup {
    ...
    wget -O - 'https://github.com/ginuerzh/gost/releases/download/v2.12.0/gost_2.12.0_linux_amd64.tar.gz' \
      | tar -xz -C ~/bin/ gost
...
#+begin_example zsh
sudo apt-get update
sudo apt-get install -y kitty-terminfo coreutils moreutils htop ncdu vim neovim git curl wget build-essential tmux zsh openconnect jq python3 python3-pip bindfs socat nmap corkscrew connect-proxy net-tools netcat-openbsd openssh-client openssh-server autossh redis-server zstd ripgrep progress make ca-certificates aria2 unzip autoconf automake libtool zlib1g-dev imagemagick ugrep eza
...
** zinit plugins
Zinit needs =aliases= because the plugins use global aliases such as =@RET=.
#+begin_src zsh :eval never
zinit ice id-as"nightsh" \
    aliases \
    pick"scripts/zshlang/basic/basic.plugin.zsh" \
    multisrc"scripts/zshlang/plugins/tmux-z/tmux-z.plugin.zsh scripts/zshlang/plugins/psiphon/psiphon.plugin.zsh scripts/zshlang/plugins/paqet/paqet.plugin.zsh"
