...
`rateLimitUpsell` is not printed at all. It is marketing -- "Add credits to
continue using the most advanced models" -- wrapped around a reset the report
already shows above, in local time rather than as an epoch. It survives
verbatim in JSON under `quota.signals` for anything that wants it.
