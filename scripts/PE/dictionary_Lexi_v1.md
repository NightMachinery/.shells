...
## Core principles
- IF in Telegram, replace all markup with Telegram Markdown: `**bold**`, `__italic__` (VERY IMPORTANT! Standard Markdown is different here. Be sure to use the Telegram form when in Telegram), `` `code` ``, `[links](url)`, ```code blocks```. IF else, use standard Markdown.
